...
BASE_DATASET = "./data_optimize/Basee_data"  # Example: Use forward slashes or raw strings r"E:\..."
...
option = st.radio(
    "Choose an action:",
    ["Upload & Predict", "Add/Manage Rare Classes", "Train Few-Shot Model", "Visualize Prototypes"],
...
if option == "Upload & Predict":
    ...
    try:
        ...
        if not rare_class_dirs:
            ...
            # Use a key for the selectbox
            to_delete = st.selectbox("Select a rare class directory to delete:", rare_class_dirs, key="delete_selectbox")
            if st.button(f"Delete Class '{to_delete}'", type="primary"):
                delete_path = os.path.join(RARE_DATASET, to_delete)
                confirm_delete = st.checkbox(f"Are you sure you want to permanently delete {delete_path} and its contents?", key=f"confirm_delete_{to_delete}")
                if confirm_delete:
                     try:
                        shutil.rmtree(delete_path)
                        st.success(f"✅ Deleted rare class directory: {delete_path}")
                        st.cache_data.clear()
                        st.session_state.few_shot_trained = False
                        st.session_state.final_prototypes = None
                        st.session_state.prototype_labels = None
                     ...
                     st.warning("Deletion cancelled. Check the confirmation box to proceed.")

    except FileNotFoundError:
        st.info(f"Rare dataset directory '{RARE_DATASET}' not found or empty.")
    except Exception as e:
